...
otel.service.name: 'my-node-functions'
```
...
exported over OTLP/HTTP as protobuf, one span per request, through the runtime's own `fetch`.
There is no OpenTelemetry SDK and no new dependency: the encoder is the engines' own
hand-written OTLP writer, ported.
...
`otel.exporter.otlp.connect.timeout` has no effect — `otel.exporter.otlp.timeout` bounds the
whole export, connect included (a warning names it); the instrumentation scope is
`mercury-composable-nodejs`, so a backend shows which runtime produced a span. The keys are
...
```yaml
livenessProbe:
  httpGet: { path: /livenessprobe, port: 8087 }
readinessProbe:
