# Mercury Composable for Node.js — consumer starting point
...
> serves AI tools that **consume the Node.js wrapper as a dependency** and need the fastest
...
`mercury-nodejs` is a lightweight Event-over-HTTP function host: write decoupled functions
in JavaScript/TypeScript and let Mercury Composable engines (Java, Rust) orchestrate them
from Event Script flows and MiniGraph knowledge graphs. Orchestration stays in the engines;
this package contributes functions plus the engines' operational conventions (configuration,
logging, trace, actuators).
...
wrapper's docs cover only the Node.js function surface.

## What lives in this repo

| Path | Role |
|------|------|
| `src/` | the `mercury-composable` package — `Platform`, `PostOffice`, `EventEnvelope`, actuator routes |
...
- `docs/guides/function-patterns.md` — (headers, body) contract, async, never-block-the-loop
- `docs/guides/join-event-script.md` — `yaml.event.over.http`, what the function sees
- `docs/guides/join-knowledge-graph.md` — `graph.task` to a Node.js target
...
- **Wrapper docs:** github.com/Accenture/mercury-nodejs
