...
class RestEntry {
    ...
    addHeaders(config, idx, type, info, addList, counters) {
        const id = config.getProperty(HEADERS + "[" + idx + "]." + ID);
    ...
    dropHeaders(config, idx, type, info, dropList, counters) {
    ...
    keepHeaders(config, idx, type, info, keepList, counters) {
    ...
    validateAuthConfig(config, idx, info) {
    ...
    validateHeaderTransformers(config, idx, headerId, info) {
        let found = false;
    ...
    parseMethods(config, idx, info, url, exact) {
    ...
    parseUrlRewrite(config, idx, info) {
    ...
    loadAuthConfig(config, idx, authEntry, info) {
        let valid = false;
