...
for await (const segment of po.stream('hello.tokens', undefined, {
  endpoint: 'http://127.0.0.1:8100/api/event', timeoutMs: 30000
})) {
  const marker = segment.headers['x-event-stream'];
  if (marker === 'data') {
    console.log(segment.body);
  } else if (marker === 'exception') {
    throw new Error(segment.body.message);
  }
  // eof: segment.body carries the trailing metadata, if any
}
```

`timeoutMs` is the idle allowance between segments. The consumer guards the dialect
...
```javascript
preload('llm.relay', { instances: 10, interceptor: true }, async (headers, event) => {
  const po = new PostOffice();
  await po.streamTo('remote.tokens', undefined, event.replyTo ?? '', {
    endpoint: 'http://peer:8085/api/event', cid: event.cid, timeoutMs: 30000
  });
});
```

`streamTo()` forwards every decoded envelope verbatim to the named LOCAL route
