...
summary: Wire a Node.js function into a MiniGraph model - graph.task to a declarative target,
...
`yaml.event.over.http` to a Node.js host. A minimal node:
...
fail fast rather than overstay ([Design](design.md)).

## Errors: the graph's error.* contract

When your function throws `AppException(code, message)` — or fails unexpectedly with
...
when one traveled. A graph can therefore route around a Node.js failure with the same
...
one `graph.task` node whose `task` names a route served by the demo app
(`hello.node` in [Getting Started](getting-started.md)), with the declarative map on
the engine pointing at `http://127.0.0.1:8087/api/event`. Deploy the graph (or dry-run
it in the Playground), `POST /api/graph/{graph-id}`, and the reply body is your
function's output — trace id visible in the Node.js host's log line.
