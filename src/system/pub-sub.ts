...
class PS {
     ...
     * @returns true or false
     */
    async createTopic(topic: string, partition = -1) {
        const result = await po.request(new EventEnvelope().setTo(PUB_SUB)
     ...
     */
    async partitionCount(topic: string) {
        const result = await po.request(new EventEnvelope().setTo(PUB_SUB).setHeader(TOPIC, topic)                           
     ...
     * @returns true or false
     */
    async publish(topic: string, headers: object, body) {
     ...
     * @returns true or false
     */
    async publishToPartition(topic: string, partition: number, headers: object, body) {
     ...
     * @returns true or false
     */
    async subscribe(topic: string, route: string, parameters: Array<string>) {
     ...
     * @returns true or false
     */
    async subscribeToPartition(topic: string, partition: number, route: string, parameters: Array<string>) {
        if (topic == route) {
