...
export class CompileFlows {
    ...
    private createFlow(name: string, flow: ConfigReader): void {
        ...
        if (typeof id == 'string' && typeof description == 'string' &&
            ...
            const entry: Flow = new Flow(id, firstTask, extState, ttlSeconds * 1000, topLevelException);
    ...
    private parseTaskList(taskCount: number, entry: Flow, flow: ConfigReader, name: string) {
        let endTaskCount = 0;
    ...
    private parseTask(entry: Flow, flow: ConfigReader, i: number, name: string): boolean {
    ...
    private addTask(entry: Flow, flow: ConfigReader, task: Task, i: number, uniqueTaskName: string, name: string) {
        const execution = flow.get(TASKS+"["+i+"]."+EXECUTION);
        if (FORK == execution) {                    
    ...
    private parseInputDataMapping(flow: ConfigReader, task: Task, i: number, uniqueTaskName: string, name: string): boolean {
        const input = flow.get(TASKS+"["+i+"]."+INPUT, []);
        const inputList = [];
    ...
    private parseOutputDataMapping(flow: ConfigReader, task: Task, i: number, uniqueTaskName: string, name: string): boolean {
        const output = flow.get(TASKS+"["+i+"]."+OUTPUT, []);  
        const execution = flow.get(TASKS+"["+i+"]."+EXECUTION); 
    ...
    private parseJoinTask(flow: ConfigReader, task: Task, i: number, uniqueTaskName: string, name: string) {
    ...
    private parsePipeline(flow: ConfigReader, task: Task, i: number, uniqueTaskName: string, name: string) {
    ...
    private parseLoopStatement(loopStatement: string, task: Task, uniqueTaskName: string, name: string) {
        ...
        const parts: Array<string> = util.split(loopStatement.substring(bracket+1), "(;)");
        if (type == FOR) {
    ...
    private parseForLoop(task: Task, parts: Array<string>, uniqueTaskName: string, name: string) {
    ...
    private parseWhileLoop(task: Task, parts: Array<string>, uniqueTaskName: string, name: string) {
        if (parts.length != 1) {
    ...
    private setDelay(entry: Flow, task: Task, delay: string, uniqueTaskName: string, name: string) {
    ...
    private addFlow(entry: Flow, name: string) {
    ...
    private handleThreePartFormat(result: Array<string>, parts: Array<string>) {
        /*
    ...
    private normalizedNegateTypeMapping(negate: string): string {
        /*
    ...
    private trimTypeQualifier(lhs: string): string {
